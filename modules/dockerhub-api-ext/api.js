...
(function () {
    ...
    module.exports = {
         ...
         * @param {String} username - the username
         * @param {String} name - the name of the repository
         * @param {Boolean} prv - if the repository should be private
         * @returns {Promise}
         */
        setRepositoryPrivacy: function (username, name, prv) {
            return new Promise(function (resolve, reject) {
                if (!username || !name || !descriptions) {
                    return reject(new Error('A username and repository name must be passed in as well as the data to set!'));
                }
                ...
                return this.makePostRequest(`repositories/${username}/${name}/privacy`, { is_private: prv }).then(resolve).catch(reject);
            }.bind(this));
        },
        /**
         ...
         * @returns {Promise}
         */
        auditHistory: function (username, name, options) {
            // If no name is passed in, then the user wants an official repository
            if (username && !name) {
                name = username;
                username = 'library';
            ...
            if (!options) {
                options = { includeRelated: true, limit: 100 };
            }

            // Make sure the username is all lowercase as per Docker Hub requirements
            username = username.toLowerCase();

            return this.makeAuditGetRequest(`action/?include_related=${options.includeRelated || 100}&limit=${options.limit || 100}&object=%2Fapi%2Frepo%2Fv1%2Frepository%2F${username}%2F${name}%2F`);
        },
        /**
         ...
         * @returns {Promise}
         */
        makeAuditGetRequest(path, extract) {
            return new Promise(function (resolve, reject) {
                let params = this.makeAuditRequestParams('get', path);
         ...
         * @returns {Object}
         */
        makeAuditRequestParams(method, path, data) {
            // Normalize the path so it doesn't start with a slash
            if (path.substr(0, 1) === '/') {
                path = path.substr(1);
            }
            ...
            let url = `https://cloud.docker.com/api/audit/v1/${path}`;
